...
const create: DataProvider["create"] = async <RecordType extends RaRecord>(
  ...
  if (resource === RESOURCE.MOVIES) {
    ...
    const imageUrl = params.data.imageUrl.startsWith("https://lh3.googleusercontent.com/d/")
      ? params.data.imageUrl
      : getGoogleDriveUrl(params.data.imageUrl);
  ...
  if (resource === RESOURCE.DIALOGUES) {
    ...
    try {
      const dialogues = params.data.dialogues as {
        speakerId: string;
